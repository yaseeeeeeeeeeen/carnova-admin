import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class Fontstyles {
  static TextStyle buttonText = GoogleFonts.outfit(
    color: Colors.white,
    fontWeight: FontWeight.w400,
    fontSize: 16,
  );

  static TextStyle vehicleCardName =
      GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.w500);
  static TextStyle vehicleCardlocation = GoogleFonts.outfit(fontSize: 15);
  static TextStyle vehicleCardtrans =
      GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w400);
  static TextStyle vehicleCardhost =
      GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.w500);
        static TextStyle vehicledetailShost =
      GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.w500);
}
